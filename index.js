...
client.on("ready", async ()=>{
  ...
  client.user.setActivity(' a server.', { type: 'WATCHING' });
	...
	client.guilds.forEach(guild =>{
		...
		client.commands.forEach(command =>{
			...
			if(!Permissions[guild.id][command.name]){
				Permissions[guild.id][command.name]={allowedroles:["all"],allowedchannels:["all"]};
...
client.on("guildCreate",  async guild =>{
	...
	if(!Permissions[guild.id]){
		...
		client.commands.forEach(async c => {
			...
			Permissions[guild.id][c.name]={allowedroles:["all"],allowedchannels:["all"]};
		});
	}
